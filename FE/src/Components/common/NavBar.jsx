    ...
    return (
        <div className={` flex sm:relative bg-richblack-900 w-screen relative z-50 h-14 items-center justify-center border-b-[1px] border-b-richblack-700 translate-y-  transition-all duration-500`}>
            <div className='flex w-11/12 max-w-maxContent items-center justify-between'>
                        ...
                        <div className=' md:hidden'>
                            <Link to='/dashboard/cart' className=' relative left-10' onClick={() => { dispatch(setProgress(100)) }} >
                                <div className=''>
                                    ...
                                    className=' fill-richblack-25 w-8 h-8' />
                                </div>
                                {
                                    totalItems > 0 && (
                ...
                <div className={`flex lg:hidden  relative gap flex-row ${token !== null && user?.accountType !== "Instructor" ? " -left-12" : ""}`}>
                    ...
                    <div ref={show} className='mobNav z-50 '>
                        <nav className=' items-center flex flex-col gap-2 absolute w-[200px] -left-[80px] -top-7  glass2' ref={show}>
                            {
                                token == null && (
                                    <Link to='/login' className='' onClick={() => { dispatch(setProgress(100)) }} >
                                        <button onClick={shownav} className=' px-[12px] py-[8px] 
                                        text-richblack-5 rounded-md '>
                                            Login
                                        </button>
                                    </Link>
                                )
                            ...
                            <div className=' mt-2 mb-4 bg-richblack-25 w-[200px] h-[2px]'></div>
                            ...
                            <div className=' mt-2 mb-2 bg-richblack-25 w-[200px] h-[2px]'></div>
                ...
                {/* Desktop Navbar */}
                <nav>
                    <ul className=' flex-row gap-x-6 text-richblack-25 gap-5 hidden lg:flex'>
                        {
                                                    ...
                                                    <div className='absolute left-[50%] top-0 -z-10 h-6 w-6 translate-x-[80%] translate-y-[-40%] rotate-45 select-none rounded bg-richblack-5'></div>
                                                    {
                                                        sublinks?.length < 0 ? (<div></div>) : (
                                                            sublinks?.map((subLink, index) => (
                                                                <Link to={`/catalog/${subLink?.name}`} key={index} className="rounded-lg bg-transparent py-4 pl-4 hover:bg-richblack-50" onClick={() => { dispatch(setProgress(30)) }}>
                                                                    <p className=''>
                                                                        {subLink?.name}
                                                                    </p>
                                                                </Link>
                        ...
                        <form onSubmit={handelSearch} className='flex items-center relative '>
                            <input value={searchValue} onChange={(e) => { setSearchValue(e.target.value) }} id='searchinput' type="text" placeholder="Search" 
